...
Fit models with all non-scaled variables, but keep FFD_s_y. When FFD_s_y is significant, effects are taken form models with FFD_mean and FFD_sd.
...
```{r}
ef1<-ggpredict(globmod_grazing1_A_ns,terms = "mean_3 [all]")
ef2<-ggpredict(globmod_grazing1_A_ns,terms = "mean_4 [all]")
ef3<-ggpredict(globmod_grazing1_A_ns,terms = "mean_5 [all]")
ef4<-ggpredict(globmod_pred_GLM1_A_ns,terms = "mean_3 [all]")
ef5<-ggpredict(globmod_pred_GLM1_A_ns,terms = "mean_5 [all]")
ef6<-ggpredict(globmod_pred_GLM1_A_ns,terms = "mean_6 [all]")
```
...
ggpredict(globmod_grazing1_A_ns,terms = "mean_3 [all]")%>%
  ...
  my_theme()+xlab("March temperature")+ylab(NULL)+
  theme(plot.title=element_text(hjust=-0.2,vjust=-3)),
# Grazing <- Temperature April
ggpredict(globmod_grazing1_A_ns,terms = "mean_4 [all]")%>%
  ggplot(aes(x,predicted))+geom_ribbon(aes(ymin=conf.low,ymax=conf.high),fill="grey")+
  geom_line(color="black",size=1)+ggtitle("C)")+
  my_theme()+xlab("April temperature")+ylab(NULL)+
  theme(plot.title=element_text(hjust=-0.2,vjust=-3)),
# Grazing <- Temperature May
ggpredict(globmod_grazing1_A_ns,terms = "mean_5 [all]")%>%
  ggplot(aes(x,predicted))+geom_ribbon(aes(ymin=conf.low,ymax=conf.high),fill="grey")+
  geom_line(color="black",size=1)+ggtitle("D)")+
  my_theme()+xlab("May temperature")+ylab(NULL)+
  theme(plot.title=element_text(hjust=-0.2,vjust=-3)),
ncol=4,left=textGrob("Predicted grazing",just="center",hjust=0.42,
...
ggsave(filename=
        ...
        plot=fig1,device="tiff",width=34,height=8.5,units="cm",dpi=100,compression="lzw")
```

```{r echo=FALSE, fig.height=4, fig.width=12, message=FALSE, warning=FALSE}
# Fig. 2: Number of seeds per flower
fig2<-grid.arrange(
  # N seeds per fl <- Individual FFD
ggpredict(globmod_seedsperfl_B_ns,terms = "FFD_s_y [all]")%>%
  ggplot(aes(x,predicted))+geom_ribbon(aes(ymin=conf.low,ymax=conf.high),fill="grey")+
  geom_line(color="black",size=1)+ggtitle("A)")+
  my_theme()+xlab("FFD (standardized within years)")+ylab(NULL)+
...
ggpredict(globmod_seedsperfl_B_ns,terms = "FFD_sd [all]")%>%
  ggplot(aes(x,predicted))+geom_ribbon(aes(ymin=conf.low,ymax=conf.high),fill="grey")+
  geom_line(color="black",size=1)+ggtitle("B)")+
  my_theme()+xlab("Yearly standard deviation of FFD")+ylab(NULL)+
...
ggpredict(globmod_seedsperfl_A_ns,terms = "mean_5 [all]")%>%
  ...
  my_theme()+xlab("May temperature")+ylab(NULL)+
  theme(plot.title=element_text(hjust=-0.2,vjust=1)),
ncol=3,left=textGrob("Predicted number of\nseeds per flower",just="center",hjust=0.42,
                     gp=gpar(fontsize=16,fontfamily="serif"),rot = 90))
ggsave(filename=
          "C:/Users/User/Dropbox/SU/Projects/lathyrus/lathyrus_ms3/results/figures/Figure_2.tiff",
        plot=fig2,device="tiff",width=28,height=8.5,units="cm",dpi=100,compression="lzw")
```

```{r echo=FALSE, fig.height=8, fig.width=12, message=FALSE, warning=FALSE}
myPalette <- colorRampPalette(brewer.pal(11, "YlOrRd"))
# Fig. 3: Seed predation
fig3<-grid.arrange(
  # Predation <- Mean FFD
ggpredict(globmod_pred_GLM1_B_ns,terms = "FFD_mean [all]")%>%
  ggplot(aes(x,predicted))+geom_ribbon(aes(ymin=conf.low,ymax=conf.high),fill="grey")+
  geom_line(color="black",size=1)+ggtitle("A)")+
  my_theme()+xlab("Yearly mean of FFD")+ylab(NULL)+
  theme(plot.title=element_text(hjust=-0.15,vjust=0)),
# Predation <- SD FFD
...
ggpredict(globmod_pred_GLM1_A_ns,terms = "mean_6 [all]")%>%
  ggplot(aes(x,predicted))+geom_ribbon(aes(ymin=conf.low,ymax=conf.high),fill="grey")+
  geom_line(color="black",size=1)+ggtitle("E)")+
  my_theme()+xlab("June temperature")+ylab(NULL)+
  theme(plot.title=element_text(hjust=-0.15,vjust=0)),
...
ggpredict(globmod_pred_GLM1_A_ns,terms = c("FFD_s_y [all]","mean_4 [all]"))%>%
  ...
  my_theme()+xlab("FFD (standardized within years)")+ylab(NULL)+
  theme(plot.title=element_text(hjust=-0.15,vjust=0))+ggtitle("F)")+
...
ncol=3,left=textGrob("Predicted seed predation",just="center",hjust=0.42,
                     gp=gpar(fontsize=16,fontfamily="serif"),rot = 90))
ggsave(filename=
          "C:/Users/User/Dropbox/SU/Projects/lathyrus/lathyrus_ms3/results/figures/Figure_3.tiff",
        plot=fig3,device="tiff",width=28,height=17,units="cm",dpi=100,compression="lzw")
```
...
ggsave(filename=
        ...
        device="tiff",width=130,height=100,units="mm",dpi=100,compression="lzw")
```
