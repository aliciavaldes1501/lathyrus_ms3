...
Significant effect of all grazing on selection.

### Graphs of interaction effects 

Graphs based on GLMMs with non-standardized interactions - to show real values of interactions in the color bars.
...
In these models, we use: fitness relativized accross years, FFD, n_fl,interactions and climatic variables standardized accross years (i.e. all variables, being either responses or predictors, standardized accross years).

PROBLEM: Interactions cannot be standardized when used as responses! So far standardized only when used as predictors. 
...
data_selag$grazing_corr_s<-(data_selag$grazing_corr-mean(data_selag$grazing_corr,na.rm=T))/
  sd(data_selag$grazing_corr,na.rm=T)
data_selag$n_seeds_per_fl_s<-(data_selag$n_seeds_per_fl-mean(data_selag$n_seeds_per_fl,na.rm=T))/
  sd(data_selag$n_seeds_per_fl,na.rm=T)
# Proportion of seeds escaping predation and grazing are used as successes/failures when being response
# variables, thus not standardized - OK?
```
...
globmod_n_seeds_per_fl_path_GLM<-glm(round(n_seeds_per_fl)~mean_3_s+mean_4_s+mean_5_s+
                                       FFD_s+n_fl_s+grazing_corr_s,data=data_selag_subset1,
                                     family="poisson",na.action="na.fail")
globmod_n_seeds_per_fl_path_GLMM<-glmer(round(n_seeds_per_fl)~mean_3_s+mean_4_s+mean_5_s+
                                       FFD_s+n_fl_s+grazing_corr_s+(1|id),data=data_selag_subset1,
                                       family="poisson",na.action="na.fail")
...
globmod_prop_seeds_esc_path_GLM<-glm(cbind(round(n_intact_seeds),round(n_pred_seeds))~
                                     mean_3_s+mean_4_s+mean_5_s+mean_6_s+n_fl_s+FFD_s,
...
globmod_grazing_path_GLM<-glm(cbind(grazing_success,grazing_failure)~mean_3_s+mean_4_s+mean_5_s+
                                n_fl_s+FFD_s,data = data_selag_subset3,
                              family="binomial",na.action="na.fail")
globmod_grazing_path_GLMM<-glmer(cbind(grazing_success,grazing_failure)~mean_3_s+mean_4_s+mean_5_s+
                                   n_fl_s+FFD_s+(1|id),data = data_selag_subset3,
                                 family="binomial",na.action="na.fail")
clusterType <- if(length(find.package("snow", quiet = TRUE))) "SOCK" else "PSOCK"
...
kable(summary(get.models(modsel_grazing_path_GLMM,subset=1)$"31")$coefficients,
      digits=c(3,3,2,3)) # Coefs best model, failed to converge, nearly unidentifiable
...
r.squaredGLMM(get.models(modsel_grazing_path_GLMM,
                         subset=1)$"31") # R square of best model, failed to converge, nearly unidentifiable
```
...
globmod_FFD_path_GLM<-lm(FFD_s~mean_3_s+mean_4_s+mean_5_s,data = data_selag_subset4,na.action="na.fail")
globmod_FFD_path_GLMM<-lmer(FFD_s~mean_3_s+mean_4_s+mean_5_s+(1|id),
...
In these models, we use: fitness relativized within years, FFD and n_fl standardized within years, interactions and climatic variables standardized accross years.

In this model, we do not include effects of temperature on FFD.

PROBLEM: Interactions cannot be standardized when used as responses! So far standardized only when used as predictors. 

# HERE!!! (but revise above)

### Number of seeds per flower

```{r message=FALSE, warning=FALSE}
globmod_n_seeds_per_fl_path_GLM_y<-glm(round(n_seeds_per_fl)~mean_3_s+mean_4_s+mean_5_s+
                                       FFD_s_y+n_fl_s_y+grazing_corr_s,data=data_selag_subset1,
                                     family="poisson",na.action="na.fail")
globmod_n_seeds_per_fl_path_GLMM_y<-glmer(round(n_seeds_per_fl)~mean_3_s+mean_4_s+mean_5_s+
                                       FFD_s_y+n_fl_s_y+grazing_corr_s+(1|id),data=data_selag_subset1,
                                       family="poisson",na.action="na.fail")
...
globmod_prop_seeds_esc_path_GLM_y<-glm(cbind(round(n_intact_seeds),round(n_pred_seeds))~
                                     mean_3_s+mean_4_s+mean_5_s+mean_6_s+FFD_s_y+n_fl_s_y,
                                     data=data_selag_subset2,family="binomial",na.action="na.fail")
...
kable(summary(get.models(modsel_prop_seeds_esc_path_GLM_y,
                         subset=1)$"63")$coefficients,digits=c(3,3,2,3)) # Coefs best model
kable(summary(get.models(modsel_prop_seeds_esc_path_GLM_y,subset=1)$"63")$coefficients,
      digits=c(3,3,2,3)) # Coefs best model, failed to converge
