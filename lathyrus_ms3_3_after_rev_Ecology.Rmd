...
```{r}
b_par<-bootMer(x=path1_mod2_glmer,FUN=fixef,nsim=10000)
```
...
Variation in climatic conditions during spring among years influences the intensity of biotic interactions. 

First, models to test the effect of spring temperatures on antagonistic interactions, while accounting for FFD and number of flowers. Using FFD and number of flowers standardized within years, temperatures standardized across years. NOT including interactions among FFD and temperatures. NOT including a random effect of year because there is only one value of temperature for each year. 
...
mod_seedpred<-glmmTMB(cbind(round(n_pred_seeds),round(n_intact_seeds))~
                     scale(mean_3)+scale(mean_4)+scale(mean_5)+scale(mean_6)+
                      FFD_s_y+n_fl_s_y+(1|id),data = data_selag,
                    family="binomial")
...
```{r message=FALSE, warning=FALSE}
mod_grazing_bb_weights<-glmmTMB(grazing_corr~scale(mean_3)+scale(mean_4)+
                                  scale(mean_5)+FFD_s_y+n_fl_s_y+(1|id),
                                data = data_selag,weights=grazing_weights,
                                family="betabinomial")
```

```{r}
pred_grazing_mean3<-ggpredict(mod_grazing_bb_weights,
                              terms = c("mean_3 [all]"))
pred_grazing_mean4<-ggpredict(mod_grazing_bb_weights,
                              terms = c("mean_4 [all]"))
pred_grazing_mean5<-ggpredict(mod_grazing_bb_weights,
                              terms = c("mean_5 [all]"))
```
...
ggsave(filename="output/figures/Fig_2_temps_grazing.tiff",
       plot=Fig_2_temps_grazing,device="tiff",width=15,height=10,units="cm",
       dpi=300,compression="lzw")
```

```{r message=FALSE, warning=FALSE}
data_selag$n_seeds_rounded<-round(data_selag$n_seeds)
mod_seedpred_bb_weights<-glmmTMB(prop_pred_seeds~scale(mean_3)+scale(mean_4)+
                                   scale(mean_5)+scale(mean_6)+FFD_s_y+n_fl_s_y+
                                   (1|id),data = data_selag,
                                 family="betabinomial",weights=n_seeds_rounded)
```

```{r}
pred_seedpred_mean3<-ggpredict(mod_seedpred_bb_weights,
                              terms = c("mean_3 [all]"))
pred_seedpred_mean5<-ggpredict(mod_seedpred_bb_weights,
                              terms = c("mean_5 [all]"))
pred_seedpred_mean6<-ggpredict(mod_seedpred_bb_weights,
                              terms = c("mean_6 [all]"))
```

```{r fig.height=4, fig.width=12, message=FALSE, warning=FALSE}
Fig_2_temps_seedpred<-grid.arrange(
  ggplot(data_selag,aes(x=mean_3,y=prop_pred_seeds))+
    geom_point(size=3,alpha=0.05)+
    geom_ribbon(data=pred_seedpred_mean3,
                aes(x,predicted,ymin=conf.low,ymax=conf.high),
                fill="firebrick2",alpha=0.3)+
    geom_line(data=pred_seedpred_mean3,aes(x,predicted),
              color="firebrick2",size=1)+
    my_theme()+xlab("Mean March (ºC)")+ylab(NULL)+
    theme(plot.margin = unit(c(0.3,0.1,0.3,0.1), "cm"))+
    theme(axis.text.y=element_blank()),
...
ggsave(filename="output/figures/Fig_2_temps_seedpred.tiff",
       plot=Fig_2_temps_seedpred,device="tiff",width=15,height=10,units="cm",
       dpi=300,compression="lzw")
```

# H3 - Part 2

Differences in the intensity of biotic interactions among years cause differences in selection on flowering time. 

In this phenotypic selection model, we use: fitness relativized within years, FFD and n_fl standardized within years,interactions standardized accross years. Main effects of interactions are not included because fitness was relativized within years. 
...
mod_int_sel_GLMM<-lmer(fitness_rel_y ~ FFD_s_y+n_fl_s_y+
                         FFD_s_y:scale(grazing_mean)+
                         FFD_s_y:scale(prop_pred_seeds_mean)+
                         (1|id),data_selag)
...
Fig_2_fitness<-ggplot(pred_fitness,aes(x,predicted,colour=group,fill=group))+
  ...
  theme(legend.position="top")+labs(colour="Yearly\nmean grazing  ")+
...
# Appendix S4: Relationships fitness-FFD for each level of mean grazing (separately for each year).
...
pred_fitness_all_df<-data.frame(pred_fitness_all)%>%
  rename(FFD_s_y=x, fitness_rel_y=predicted,grazing_mean=group)%>%
...
ggplot(data=subset(data_grazing_cats))+
  ...
  theme(legend.position="top")+labs(colour="Yearly mean grazing")+
  xlab("First flowering date")+ylab("Preditect fitness (number of intact seeds)")+
  theme(strip.text.x=element_text(margin=margin(2,0,2,0)))+
  guides(fill=FALSE)
ggsave(filename="output/figures/AppendixS4_A.tiff",device="tiff",width=168,
       height=210,units="mm",dpi=300,compression="lzw")
...
ggplot(data=subset(data_grazing_cats))+
  ...
  scale_fill_viridis(direction = -1)+
  theme(legend.position="top")+labs(colour="Yearly mean grazing")+
  xlab("First flowering date")+ylab("Predicted fitness (number of intact seeds)")+
  theme(strip.text.x=element_text(margin=margin(2,0,2,0)))+
  guides(fill=FALSE)
ggsave(filename="output/figures/AppendixS4_B.tiff",device="tiff",width=168,
       height=210,units="mm",dpi=300,compression="lzw")
```
...
# Effects of variance in temperatures on interactions
